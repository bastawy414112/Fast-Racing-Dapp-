    ...
    IRacing
{
    ...
    mapping(RaceMode => mapping(uint256 => uint256))
        private raceIdModeToRandomRequestId;
    mapping(RaceMode => mapping(uint256 => bytes32))
        private raceIdModeToFunctionsRequestId;
    ...
    ) VRFConsumerBaseV2Plus(_vrfCoordinator) FunctionsClient(_router) {
        COORDINATOR = IVRFCoordinatorV2Plus(_vrfCoordinator);
    }

    function getRandomRequestFromID(
        uint256 id
    ) public view returns (RandomRequests memory) {
        return requestIdToRandomRequests[id];
    }

    function getFunctionsRequestFromID(
        bytes32 id
    ) public view returns (FunctionsRequests memory) {
    ...
    function requestRandomNumber(
        ...
        requestIdToRandomRequests[requestId] = RandomRequests({
            fulfilled: false,
            exists: true,
        ...
        returns (bytes32 _requestId)
    {
        FunctionsRequest.Request memory req;
        req.initializeRequestForInlineJavaScript(
            FunctionsSource.getWeatherScore()
        );
    ...
    function fulfillRequest(
        ...
        FunctionsRequests memory _request = requestIdToFunctionsRequests[
            requestId
        ];
        ...
        if (_request.requestType == RequestType.WEATHER_SCORE) {
            ...
            _finishRace(
                requestIdToRaceId[requestId],
                requestIdToRaceMode[requestId],
                values
            );
        } else {
     ...
     */
    function _formatCircuitIndex(
        uint256 _circuitIndex
    ) private pure returns (string memory) {
        if (_circuitIndex == 0) {
        ...
        if (adjustedIndex < 10) {
            return
                string(abi.encodePacked("0", Strings.toString(adjustedIndex)));
        } else {
     ...
     * decimal)
     */
    function _formatPlayerAttributes(
    ...
    function _updateWeatherDataForCircuit(
        uint256 circuit,
        uint256 weatherScore
    ) internal virtual;
}
